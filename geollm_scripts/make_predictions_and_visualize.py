...
def get_local_prediction(tokenizer, model, prompt):
    ...
    return completion, rating, None

def load_local_model(device, model_name):
...
def run_task_for_data(model_api, model, task, prompt_file_path, api_key):
    ...
    if model_api == "local":
        ...
        tokenizer, local_model = load_local_model(api_key, model)
    ...
    for prompt in prompts:
        try:
            ...
            if model_api == "openai":
                ...
                completion, most_probable, expected_value = get_local_prediction(tokenizer, local_model, prompt)
            else:
