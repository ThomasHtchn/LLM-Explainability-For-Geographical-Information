...
# Does a forwardpass and return the hidden states of the model
...
# Prints the top-k tokens from each hidden layers, decoded using model.lm_head (norm?)
def print_top_tokens_per_layer(prompt, expected_next_token, model, tokenizer, is_already_tokenized=False, start_layer=30, end_layer=36, top_k=20):
    ...
    for i, h in enumerate(hidden_states[start_layer:end_layer+1], start=start_layer):
        ...
        for j, tid in enumerate(top_token_ids[0]):
            ...
            if(tid.item() == tokenizer.encode(expected_next_token)[0] or remove_spaces(decoded_token) == remove_spaces(expected_next_token)):
                print(f"\n /!\\ ALERT : {expected_next_token} detected -> top {j+1} /!\\ \n")
